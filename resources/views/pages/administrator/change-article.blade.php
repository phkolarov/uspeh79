                                ...
                                @if( $filter == 'Новини')
                                    <div class="col-md-2 pull-right">
                                        <a class="btn btn-primary"
                                           href="{{route('change-current-article',['id'=> 'new'])}}">Нова новина</a>
                                    </div>
                                @endif
                            </div>
                            <br>
                            <table class="table list-of-articles">
                                ...
                                @foreach($articles as $article)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$article->getCategory()->orderBy('id')->get()->first()->name}}</td>
                                        <td><a href="{{route('change-current-article',['id'=> $article->id])}}"
...
@section('javascript')
    <script>
        ...
        $('#article-filter').on('change', function () {
            location.href = "administrator/change-article/" + $(this).val();
        });
